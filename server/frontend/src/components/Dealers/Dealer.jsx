...
//           reviews.map(review => (
//             <div className='review_panel' key={review.id}> {/* Added key for each review */}
...
const Dealer = () => {
  ...
  const get_dealer = async () => {
    ...
    if (retobj.status === 200) {
      // CORRECTED PART: Setting the entire dealer object directly
  ...
  return (
    <div style={{ margin: "20px" }}>
      ...
      <div style={{ marginTop: "10px" }}>
        ...
        <h4 style={{ color: "grey" }}>{dealer.city ? `${dealer.city}, ${dealer.address}, Zip - ${dealer.zip}, ${dealer.state}` : "Loading..."}</h4>
      </div>
      <div className="reviews_panel">
        {reviews.length === 0 && unreviewed === false ? (
          <text>Loading Reviews....</text>
